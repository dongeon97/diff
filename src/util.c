...
/* Queue up filenames to be printed at the end,
...
/* Heads of chains of filenames.  */
static struct filename *created_chain;
static struct filename *modified_chain;
static struct filename *deleted_chain;

/* Tails of chains of filenames.  */
static struct filename **created_chain_end = &created_chain;
static struct filename **modified_chain_end = &modified_chain;
static struct filename **deleted_chain_end = &deleted_chain;

void enqueue_filename (struct filename **chain, struct filename ***chain_end, char const *name) {
  struct filename *new = xmalloc (sizeof *new);

  new->name = xmalloc (strlen(name) + 1);
  snprintf(new->name, strlen(name) + 1, "%s", name);
...
void enqueue_created_filename (char const *name) {
  enqueue_filename (&created_chain, &created_chain_end, name);
}

void enqueue_modified_filename (char const *name) {
  enqueue_filename (&modified_chain, &modified_chain_end, name);
}

void enqueue_deleted_filename (char const *name) {
  enqueue_filename (&deleted_chain, &deleted_chain_end, name);
}

/* Print the filenames in chain */
char * print_filename_chain (struct filename *chain) {
  char buffer[1024];
  int idx = 0;

  for (struct filename *f = chain; f; ) {
    idx += sprintf(buffer + idx, "%s, ", f->name);
    
    struct filename *next = f->next;
    free (f);
    f = next;
  }
...
void print_created_filename_chain (void) {
  printf("Created files: %s\n", print_filename_chain (created_chain));
}

void print_modified_filename_chain (void) {
  printf("Modified files: %s\n", print_filename_chain (modified_chain));
}

void print_deleted_filename_chain (void) {
  printf("Deleted files: %s\n", print_filename_chain (deleted_chain));
}

/* Output the filenames saved up by calls to 'enqueue_*_filename'.  */
void print_all_filename_chain (void) {
  print_created_filename_chain ();
  print_modified_filename_chain ();
  print_deleted_filename_chain ();
}
