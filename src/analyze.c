...
static void
briefly_report (int changes, struct file_data const filevec[], bool silent)
{
  if (changes)
      if(silent){
        char *temp = xmalloc(strlen(filevec[0].name) + 1);
        strcpy(temp, filevec[0].name);
  ...
  if (read_files (cmp->file, files_can_be_treated_as_binary))
    {
      ...
      briefly_report (changes, cmp->file, cmp->silent);
    }
  else
    {
      struct context ctxt;
