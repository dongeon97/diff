...
/* Categorize outputs. */
static bool output_categorized;
static bool output_created_only;
static bool output_modified_only;
static bool output_deleted_only;
static bool silent;

...
enum
{
  ...
  /* These options are for printing the output in a categorized manner. */
  OUTPUT_CATEGORIZED,
  OUTPUT_CREATED_ONLY,
  OUTPUT_MODIFIED_ONLY,
  OUTPUT_DELETED_ONLY,
};
...
static struct option const longopts[] =
{
  ...
  {"list", 0, 0, OUTPUT_CATEGORIZED},
  {"created", 0, 0, OUTPUT_CREATED_ONLY},
  {"modified", 0, 0, OUTPUT_MODIFIED_ONLY},
  {"deleted", 0, 0, OUTPUT_DELETED_ONLY},
...
main (int argc, char **argv)
{
  ...
  while ((c = getopt_long (argc, argv, shortopts, longopts, nullptr)) != -1)
    {
      switch (c)
        ...
        case OUTPUT_CATEGORIZED:
          output_categorized = true;
          break;
        
        case OUTPUT_CREATED_ONLY:
          output_created_only = true;
          break;
        
        case OUTPUT_MODIFIED_ONLY:
          output_modified_only = true;
          break;

        case OUTPUT_DELETED_ONLY:
          output_deleted_only = true;
          break;
  ...
  if (output_categorized || output_created_only || output_modified_only || output_deleted_only){
    silent = true;
  }
  ...
  if (output_categorized) {
    print_all_filename_chain();
  }
  else if (output_created_only) {
    print_created_filename_chain();
  }
  else if (output_modified_only) {
    print_modified_filename_chain();
  }
  else if (output_deleted_only) {
...
#define DIR_P(f) (S_ISDIR (cmp.file[f].stat.st_mode) != 0)
  ...
  if (! ((name0 && name1)
         ...
         || new_file))
    {
      ...
      if (silent) {
        char *temp = xmalloc(strlen(dir) + strlen(name) + 2);
        snprintf(temp, strlen(dir) + strlen(name) + 2, "%s/%s", dir, name);
        if (name0)
          enqueue_deleted_filename(temp);
        if (name1)
          enqueue_created_filename(temp);
      }
      else {
          /* See POSIX 1003.1-2001 for this format.  */
          message ("Only in %s: %s\n", dir, name);
  ...
  if (status != EXIT_SUCCESS)
    ...
    {
      if (silent){
        // Currently only printing the filename of dir1.
        char *temp = xmalloc(strlen(cmp.file[0].name) + 1);
        strcpy(temp, cmp.file[0].name);
        enqueue_modified_filename(temp);
      } else {
